...
class GUI(App):
    ...
    def on_start(self): #variables
        ...
        self.pinPoint = self.root.get_screen('mainScreen').ids.pp
    ...
    def cycle(self, readCYCLETIME):
        ...
        if not self.graphDone:
            ...
            [self.shapeX, self.shapeY, self.graphDone] = self.circle.circleGraph(200, 600, 433, 7)#radius, offsets, time
            self.pinPoint.xPos = self.shapeX
            self.pinPoint.yPos = self.shapeY
        #calc angles
    ...
    def drawingAlgorithm(self): #one itteration of drawAlg. actually the movment alg. drawing is done in cycle atm
        #finding length of a(dictance form origo to endpoint)
        self.a = math.sqrt(((self.shapeX-self.origoX)**2) + ((self.shapeY-self.origoY)**2))
        ...
        self.B = math.degrees(math.atan((self.shapeY-self.origoY) / (self.shapeX-self.origoX)))#simpel trigonometry
